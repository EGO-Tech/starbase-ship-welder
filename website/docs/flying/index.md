...
To enter the ship, there is are buttons on each of the ship that will open `Door` towards the front of the ship and `Service` towards the back of the ship. Once inside you can also open and close the cockpit glass `Canopy`. Buttons for these are available in the pilot right side console.

## Controlling the Ship

![Pilot Center Console](./img/pilot_center_console.png)
...
| `StrengthFactor` | Current strength of ship. Anything below 1.0 means ship is damaged. |
...
![Pilot Left Console](img/pilot_left_console.png)
...
Waypoint 1 to 6 are presets and can not be over written.
...
`Nav` may not be very accurate. I recommend using it in conjunction with the NavGrid compass. The `dd` panel on the pilot left console shows debugging information for NavCas.
...
For safety, `Avoidance` will not activate if the `Canopy` is open.
...
There is very limited coverage of range finders for the crash avoidance system. **Use at your own risk!**
...
![Pilot Right Console](img/pilot_right_console.png)
